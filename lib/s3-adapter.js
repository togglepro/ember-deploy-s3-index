...
module.exports = CoreObject.extend({
  init: function() {
    ...
    // Using `prefix` to store index in a folder needs "indirect" as `indexMode`
    if (this.indexPrefix.indexOf('/') >= 0 && this.indexMode !== 'indirect') {
      throw new Error("You need to set 'indirect' indexMode for using prefix to store revisions in a folder");
    }
  },
  ...
  activate: function(revision) {
    var revisionWithPrefix = this.indexPrefix.concat(revision);

    return this._getBucketContents()
      .then(this._getBucketRevisions.bind(this))
      .then(this._activateRevision.bind(this, revisionWithPrefix))
  ...
  _activateRevision: function(revision, currentRevisions) {
    this.ui.writeLine("_activateRevision");
    if (currentRevisions.indexOf(revision) > -1) {
      return new Promise(function(resolve, reject) {
        this._getFileContents(revision + '.html')
        .then(function(contents) {
          if (this.indexMode === 'direct') {
            return this._updateBucketWebsite(revision);
          } else {
            var params = this._getUploadParams('index', contents)
            return this._setFileContents(params);
          }
        }.bind(this))
        .then(resolve)
        .catch(function(err) {
  ...
  _removeCurrentRevisionFromContents: function(data) {
    for (var i = 0, len = data.Contents.length; i < len; i++) {
      var fileName = data.Contents[i].Key.replace(this.indexPrefix, '');
